...
def plot_mnist_image(image, label, ax=None, axis="off"):
    ax = plt.gca() if ax == None else ax
...
def plot_mnist_images(images, labels, n):
    
    if n == 1:
        plot_mnist_image(image[0], label[0], ax=None, axis="on")
        plt.show()
        return
    ...
    if n < 6:
        ...
        fig, axes = plt.subplots(int(np.ceil(n/6)), 6)

    for image, label, ax in zip(images, labels, axes.flatten()):
        plot_mnist_image(image, label, ax=ax, axis="off")
