...
def balance(df, label_feature, seed=None):
    # balancing dataset
    label_values = set(df[label_feature])
    num_samples = min( df[label_feature].value_counts() )
    for i, value in enumerate(label_values):
        sample = df.loc[ df[label_feature] == value ].sample(n=num_samples, random_state=seed)
        balanced_df = sample if i == 0 else balanced_df.append( sample )
    return balanced_df
