...
def predict(model, image):
    image = torch.tensor(image).unsqueeze(0)                    # need to add a dimension for the batch
    seq = image.permute(1, 0, 2)                                # need shape (seq_len, batch_size, input_size)
...
def display_prediction(prediction):
    ...
    for i, prob in enumerate(prediction):
        print(f"{i}:\t{float(prob):.4f}\t{'<--' if i == p else ''}")
